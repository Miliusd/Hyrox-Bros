...
export const getMembers = cache(async () => { const supabase = await createClient(); const { data, error } = await supabase.from("profiles").select("id,display_name,emoji,division,role").order("display_name"); if (error) throw error; return data; });
