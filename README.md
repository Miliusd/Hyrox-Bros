...
2. Apply the numbered files in `supabase/migrations/` in order, then edit the five usernames and names in `supabase/seed.sql` and run it.
3. Run `npm run dev`.

Only usernames represented in `allowed_members` can create a profile. The app stores them internally as `username@hyroxbros.local`; no real email is used. One member should have the `coach` role.
